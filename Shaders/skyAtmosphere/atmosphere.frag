...
void main()
{
    ...
    float camHeightScale = camWidthScale*iChannelResolution.y/iChannelResolution.x;
    
    vec3 camRight = normalize(cross(camDir, vec3(0.0, 1.0, 0.0)));
    vec3 camUp = normalize(cross(camRight, camDir));
    
    vec2 xy = 2.0 * (fragCoord.xy / iChannelResolution.xy) - 1.0;
    ...
    if (length(sunLum) > 0.0) {
        if (rayIntersectSphere(viewPos, rayDir, groundRadiusMM) >= 0.0) {
            ...
            sunLum *= getValFromTLUT(skyLUT, iChannelResolution.xy, viewPos, sunDir);
        }
    }
    lum += sunLum;
