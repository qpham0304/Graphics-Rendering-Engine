...
int BasicRayTracing::show_demo()
{
    ...
    materials.push_back({ glm::vec3(0.8, 0.5, 1.0), 0.5, 1.0 });
    materials.push_back({ glm::vec3(0.0, 1.0, 1.0), 0.8, 1.0 });
    materials.push_back({ glm::vec3(1.0, 1.0, 0.0), 0.4, 1.0 });
    materials.push_back({ glm::vec3(69/255.0, 26/255.0, 26/255.0), 0.6, 1.0 });
    materials.push_back({ glm::vec3(0.3, 0.3, 0.3), 0.3, 1.0 });
    ...
    while (!glfwWindowShouldClose(SceneRenderer::window)) {
        ...
        rayTracingFBO.Bind();
        accumulateEnabled ? frameIndex++ : frameIndex = 1;
        //if(frameIndex == 0) {
            std::cout << frameIndex << "\n";
            glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
        //}
        ...
        accumulateShader.setInt("previousScene", 0);
